...
> [!Note]
>
> Content controls augment rather than replace the identity and perimeter layers; each layer is designed assuming the others can fail. A compromised identity is still bounded by the perimeter; a payload that bypasses the perimeter is still inspected for shape and intent.
