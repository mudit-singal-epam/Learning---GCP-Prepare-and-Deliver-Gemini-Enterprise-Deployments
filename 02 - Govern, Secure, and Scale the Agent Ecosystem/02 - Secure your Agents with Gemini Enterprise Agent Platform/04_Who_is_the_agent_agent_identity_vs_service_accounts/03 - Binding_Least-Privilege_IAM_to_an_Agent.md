...
>
> Grant the narrowest role on the narrowest resource, bound to the agent's own identity, never a shared account. Treat PAB as the safeguard that contains misconfiguration, so a single wrong binding never opens the whole organization.
