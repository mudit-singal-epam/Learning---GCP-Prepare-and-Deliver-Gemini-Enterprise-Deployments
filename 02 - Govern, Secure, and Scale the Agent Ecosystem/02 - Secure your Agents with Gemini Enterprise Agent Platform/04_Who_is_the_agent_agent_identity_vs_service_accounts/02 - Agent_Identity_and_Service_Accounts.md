...
1. **Susceptibility to Excess Permissions**
...
2. **Lifecycle Decoupled From the Workload**
...
3. **Long-Lived Keys When Exported**
...
```python
# Deploy to Agent Runtime with Agent Identity enabled
remote_app = client.agent_engines.create(
    agent=app,
    config={
        "identity_type": "AGENT_IDENTITY",  # Enable Agent Identity
        "display_name": "Warranty Claim Agent",
    },
)
```