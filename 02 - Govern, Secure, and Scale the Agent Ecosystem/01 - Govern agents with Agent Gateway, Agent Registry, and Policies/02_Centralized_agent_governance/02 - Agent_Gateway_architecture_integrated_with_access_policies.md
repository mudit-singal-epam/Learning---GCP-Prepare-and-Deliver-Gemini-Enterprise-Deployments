...
The marker numbers in the diagram illustrate the request flow through the gateway:

![alt text](image-1.png)

1. Call out (mTLS via PSC Attachment)
2. Headers OK
3. Prompt Sanitized (Forwards to Endpoint)
4. Tool Output Returned
5. Masked & Verified Response (Returns to Agent)
...
*The gateway supports two policy profiles.*

1. **REQUEST_AUTHZ (Request Authorization)**: This extension is evaluated at the HTTP header stage before the request is forwarded. It integrates with Identity-Aware Proxy (IAP) to validate whether the agent's identity holds appropriate permissions to call the specific target endpoint or tool.
2. **CONTENT_AUTHZ (Content Authorization)**: This extension streams the request and response bodies to a service extension, typically [Model Armor](https://docs.cloud.google.com/gemini-enterprise-agent-platform/govern/configure-model-armor), to sanitize prompts, detect injection attacks, and block sensitive data leaks.
